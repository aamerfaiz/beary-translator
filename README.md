...
## If translation fails with a network/CORS error

Most AI APIs (DeepSeek included) block direct requests from web pages for
security reasons, so you *might* see a "Failed to fetch" style error. If
that happens, you'll need a small local relay to forward requests from the
browser to the API — none is bundled with this build yet.
